...
enum
{
	...
	BARNEY_WEAPON_DEAGLE,
	BARNEY_WEAPON_PYTHON,
};

enum
{
	BARNEY_BODY_GUN_GLOCK_HOLSTERED = 0,
	BARNEY_BODY_GUN_GLOCK_ARMED,
	BARNEY_BODY_GUN_DEAGLE_HOLSTERED,
	BARNEY_BODY_GUN_DEAGLE_ARMED,
	BARNEY_BODY_GUN_PYTHON_HOLSTERED,
	BARNEY_BODY_GUN_PYTHON_ARMED,
	BARNEY_BODY_GUN_NONE,
};

enum
{
	BARNEY_BODY_CLOTHES_NORMAL = 0,
	BARNEY_BODY_CLOTHES_CASUAL,
};
enum
{
	BARNEY_BODY_PROP_NONE = 0,
	BARNEY_BODY_PROP_DONUT,	
	BARNEY_BODY_PROP_CLIPBOARD,
};

enum
{
	BARNEY_BODY_GUN_GROUP = 1,
	BARNEY_BODY_CLOTHES_GROUP,
	BARNEY_BODY_PROP_GROUP,
};
...
void COtis :: OtisFirePistol ( void )
{
	...
	if (pev->weapons == BARNEY_WEAPON_DEAGLE)
	{
		snd = "weapons/desert_eagle_fire.wav";
		bullet = BULLET_PLAYER_357;
	}
	else if (pev->weapons == BARNEY_WEAPON_PYTHON)
	{
	...
	EMIT_SOUND_DYN( ENT(pev), CHAN_WEAPON, snd, 1, ATTN_NORM, 0, 100 + pitchShift );
...
void COtis :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	case OTIS_AE_DRAW:
		...
		if (pev->weapons == BARNEY_WEAPON_DEAGLE)
		{
			if (GetBodygroup(BARNEY_BODY_GUN_GROUP) == BARNEY_BODY_GUN_DEAGLE_HOLSTERED)
			{
				SetBodygroup(BARNEY_BODY_GUN_GROUP, BARNEY_BODY_GUN_DEAGLE_ARMED);
				m_fGunDrawn = true;
			}
		}
		else if (pev->weapons == BARNEY_WEAPON_PYTHON)
		{
			if (GetBodygroup(BARNEY_BODY_GUN_GROUP) == BARNEY_BODY_GUN_PYTHON_HOLSTERED)
			{
				SetBodygroup(BARNEY_BODY_GUN_GROUP, BARNEY_BODY_GUN_PYTHON_ARMED);
				m_fGunDrawn = true;
			}
		}
		else
		{
			if (GetBodygroup(BARNEY_BODY_GUN_GROUP) == BARNEY_BODY_GUN_GLOCK_HOLSTERED)
			{
				SetBodygroup(BARNEY_BODY_GUN_GROUP, BARNEY_BODY_GUN_GLOCK_ARMED);
				m_fGunDrawn = true;
			}
		}
	...
	case OTIS_AE_HOLSTER:
		...
		if (pev->weapons == BARNEY_WEAPON_DEAGLE)
		{
			if (GetBodygroup(BARNEY_BODY_GUN_GROUP) == BARNEY_BODY_GUN_DEAGLE_ARMED)
			{
				SetBodygroup(BARNEY_BODY_GUN_GROUP, BARNEY_BODY_GUN_DEAGLE_HOLSTERED);
				m_fGunDrawn = false;
			}
		}
		else if (pev->weapons == BARNEY_WEAPON_PYTHON)
		{
			if (GetBodygroup(BARNEY_BODY_GUN_GROUP) == BARNEY_BODY_GUN_PYTHON_ARMED)
			{
				SetBodygroup(BARNEY_BODY_GUN_GROUP, BARNEY_BODY_GUN_PYTHON_HOLSTERED);
				m_fGunDrawn = false;
			}
		}
		else
		{
			if (GetBodygroup(BARNEY_BODY_GUN_GROUP) == BARNEY_BODY_GUN_GLOCK_ARMED)
			{
				SetBodygroup(BARNEY_BODY_GUN_GROUP, BARNEY_BODY_GUN_GLOCK_HOLSTERED);
				m_fGunDrawn = false;
			}
		}
		break;
...
void COtis :: Spawn()
{
	...
	if (pev->weapons == 0)
	{
		pev->weapons = RANDOM_LONG(BARNEY_WEAPON_GLOCK, BARNEY_WEAPON_PYTHON);
	}
	...
	SetBodygroup( BARNEY_BODY_CLOTHES_GROUP, m_iOtisHead );
	SetBodygroup( BARNEY_BODY_PROP_GROUP, m_iOtisProp );
...
{
	if ( !(GetBodygroup( BARNEY_BODY_GUN_GROUP ) == BARNEY_BODY_GUN_NONE) )
	{// drop the gun!
		Vector vecGunPos;
		Vector vecGunAngles;

		SetBodygroup( BARNEY_BODY_GUN_GROUP, BARNEY_BODY_GUN_NONE );
...
void COtis::KeyValue( KeyValueData* pkvd )
{
	...
	if (FStrEq("prop", pkvd->szKeyName))
	{
		m_iOtisProp = atoi(pkvd->szValue);
		pkvd->fHandled = true;
	}
	else
	{
