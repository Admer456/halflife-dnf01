...
void CEagle::Precache()
{
	...
	UTIL_PrecacheOther("projectile_bullet");
}
...
void CEagle::PrimaryAttack()
{
	...
	const float flSpread = m_bLaserActive ? 0.6 : 2.06;
	Vector vecSpread = Vector(RANDOM_FLOAT(-flSpread, flSpread), RANDOM_FLOAT(-flSpread, flSpread), RANDOM_FLOAT(-flSpread, flSpread));
...
/*	PLAYBACK_EVENT_FULL(
		...
		g_vecZero, g_vecZero,
		anglesAim.x, anglesAim.y,
		0, 0,
		m_iClip == 0, 0 ); */

	SendWeaponAnim(EAGLE_SHOOT);
	EMIT_SOUND_DYN(ENT(pev), CHAN_WEAPON, "weapons/desert_eagle_fire.wav", 1.0, ATTN_NORM, 0, 90 + (RANDOM_FLOAT(-10, 10)));
	m_pPlayer->pev->punchangle.x += RANDOM_FLOAT(1.0, 3.5);
