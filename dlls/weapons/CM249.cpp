...
/*	Vector vecDir = m_pPlayer->FireBulletsPlayer( 
	...
	CProjectileBullet *pBullet = CProjectileBullet::LaunchProjectile(WDT_556, m_pPlayer);
...
/*	PLAYBACK_EVENT_FULL( 
	...
	{
	case 0:	SendWeaponAnim(M249_SHOOT1); break;
	case 1:	SendWeaponAnim(M249_SHOOT2); break;
	case 2:	SendWeaponAnim(M249_SHOOT3); break;
	}

	EMIT_SOUND_DYN(ENT(pev), CHAN_WEAPON, "weapons/saw_fire1.wav", 1.0, ATTN_NORM, 0, 90 + (RANDOM_FLOAT(-10, 10)));
	m_pPlayer->pev->punchangle.x += RANDOM_FLOAT(1.0, 5.5);
