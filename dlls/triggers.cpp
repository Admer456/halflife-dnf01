...
class CTriggerNoWeaponZone : public CBaseEntity
{
public:
	void Spawn( void );
	void KeyValue( KeyValueData *pkvd );
...
LINK_ENTITY_TO_CLASS( trigger_noweaponzone, CTriggerNoWeaponZone );

void CTriggerNoWeaponZone::Spawn()
{
	pev->solid = SOLID_TRIGGER;
	pev->movetype = MOVETYPE_PUSH;
...
//	SetTouch( &CTriggerNoWeaponZone::Touch );
}

void CTriggerNoWeaponZone::KeyValue( KeyValueData *pkvd )
{
	CBaseEntity::KeyValue( pkvd );
}

void CTriggerNoWeaponZone::Touch( CBaseEntity *pOther )
{
	ALERT( at_console, "\ntrigger_noweaponzone, touching" );

	if ( pOther->IsPlayer() )
	{
		CBasePlayer *pPlayer = (CBasePlayer*)pOther;
		CBasePlayerItem *pItem = pPlayer->m_pActiveItem;
		pPlayer->m_fNoWeaponZone = true;

		if ( pItem->m_iId != WEAPON_EMPTYHANDS )
			pPlayer->SelectItem( "weapon_nothing" );
	}
}

// trigger_noweaponzone point entity variant
class CTriggerNoWeaponPoint : public CBaseEntity
{
public:
	void Spawn( void );
	void KeyValue( KeyValueData *pkvd );
...
LINK_ENTITY_TO_CLASS( trigger_noweaponpoint, CTriggerNoWeaponPoint );

void CTriggerNoWeaponPoint::Spawn()
{ }

void CTriggerNoWeaponPoint::KeyValue( KeyValueData *pkvd )
{
	CBaseEntity::KeyValue( pkvd );
}

void CTriggerNoWeaponPoint::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	ALERT( at_console, "\ntrigger_noweaponzone, touching" );
	...
	if ( pActivator->IsPlayer() )
	{
		...
		if ( pPlayer->m_fNoWeaponZone != 2 )
			pPlayer->m_fNoWeaponZone = 2;
		else
			pPlayer->m_fNoWeaponZone = 0;
...
extern int gmsgPlayerTalk;

// trigger_playertalk - activates the protagonist's voice
class CTriggerPlayerMonologue : public CBaseEntity
{
public:
	void Spawn( void );
	void Precache( void );
...
LINK_ENTITY_TO_CLASS( trigger_playertalk, CTriggerPlayerMonologue );

void CTriggerPlayerMonologue::Spawn( void )
{ 
	Precache();
}

void CTriggerPlayerMonologue::Precache( void )
{
	PRECACHE_SOUND( (char*)STRING( m_iszMessage ) );
}

void CTriggerPlayerMonologue::KeyValue( KeyValueData *pkvd )
{
	if ( FStrEq( pkvd->szKeyName, "msg" ) )
	{
...
void CTriggerPlayerMonologue::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	if ( pActivator->IsPlayer() )
	{
		MESSAGE_BEGIN( MSG_ONE, gmsgPlayerTalk, NULL, pActivator->pev );
