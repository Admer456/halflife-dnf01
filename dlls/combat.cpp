...
extern char *szPlayerHurtLines[];
extern char *szPlayerAttackKillLines[];
...
int CBaseMonster :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	...
	if ( IsPlayer() )
	{
		...
		static_cast<CBasePlayer*>(this)->PlayerMonologue( szPlayerHurtLines, 0.15, 2 );
	}
	...
	if ( !IsPlayer() )
	{
		CBaseEntity *ent = CBaseEntity::Instance( pevAttacker );

		if ( ent->IsPlayer() )
		{
			static_cast<CBasePlayer*>(ent)->PlayerMonologue( szPlayerAttackKillLines, 0.2, 2 );
		}
	}
