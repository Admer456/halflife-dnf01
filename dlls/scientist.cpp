...
#define		NUM_SCIENTIST_SKINS			2 // two skin available for scientist model


enum
{
	SCI_BODY_HEAD_GLASSES = 0,
	SCI_BODY_HEAD_EINSTEIN,
	SCI_BODY_HEAD_LUTHER,
	SCI_BODY_HEAD_SLICK,
};

enum
{
	SCI_BODY_NEEDLE_OFF = 0,
	SCI_BODY_NEEDLE_ON,
};

enum
{
	SCI_BODY_HEAD_GROUP = 0,
	SCI_BODY_NEEDLE_GROUP,

	CIV_BODY_GROUP_CLOTHES = 0,
	CIV_BODY_GROUP_HEAD,
};

enum
{
	SCI_DIALOG_DEFAULT = 0,
	SCI_DIALOG_CIVILIAN_FEMALE,
};
...
enum eSciClass
{
	...
	SciClass_Russian,
	SciClass_Rosenberg,
	SciClass_CivBusiness,
	SciClass_CivOffice,
	SciClass_CivSchoolgirl,
	SciClass_CivWoman
};
...
LINK_ENTITY_TO_CLASS( monster_scientist_russian, CScientist );
LINK_ENTITY_TO_CLASS( monster_rosenberg, CScientist );
LINK_ENTITY_TO_CLASS( monster_civilian_businessman, CScientist );
LINK_ENTITY_TO_CLASS( monster_civilian_officeworker, CScientist );
LINK_ENTITY_TO_CLASS( monster_civilian_schoolgirl, CScientist );
LINK_ENTITY_TO_CLASS( monster_civilian_woman, CScientist );
...
void CScientist::DeclineFollowing( void )
{
	...
	if (m_iSciDialog == SCI_DIALOG_CIVILIAN_FEMALE)
		PlaySentence( "WO_POK", 2, VOL_NORM, ATTN_NORM );
	else 
...
void CScientist :: Scream( void )
{
	if ( FOkToSpeak() )
	{
		...
		if (m_iSciDialog == SCI_DIALOG_CIVILIAN_FEMALE)
			PlaySentence( "WO_SCREAM", RANDOM_FLOAT(3, 6), VOL_NORM, ATTN_NORM );
		else 
...
//		if ( FOkToSpeak() )
		...
		if (m_iSciDialog == SCI_DIALOG_CIVILIAN_FEMALE)
			PlaySentence( "WO_HEAL", 2, VOL_NORM, ATTN_IDLE);
		else 
		...
		if ( FOkToSpeak() )
		{
			...
			if (m_iSciDialog == SCI_DIALOG_CIVILIAN_FEMALE)
			{
				if ( m_hEnemy->IsPlayer() )
					PlaySentence( "WO_PLFEAR", 5, VOL_NORM, ATTN_NORM );
				else 
					PlaySentence( "WO_FEAR", 5, VOL_NORM, ATTN_NORM );
			}
			else
			{
				if ( m_hEnemy->IsPlayer() )
...
void CScientist :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	case SCIENTIST_AE_NEEDLEON:
		{
		SetBodygroup(SCI_BODY_NEEDLE_GROUP, SCI_BODY_NEEDLE_ON);
		}
		break;
	case SCIENTIST_AE_NEEDLEOFF:
		{
		SetBodygroup(SCI_BODY_NEEDLE_GROUP, SCI_BODY_NEEDLE_OFF);
		}
		break;
...
{
	if (FStrEq("canheal", pkvd->szKeyName))
	{
		m_iSciCanHeal = atoi(pkvd->szValue);
		pkvd->fHandled = true;
	}
	else
	{
...
void CScientist :: Spawn( void )
{
	...
	bool skipcosmeticstuff = false;

	if (FStrEq(STRING(pev->classname), "monster_rosenberg"))
	{
		SET_MODEL(ENT(pev), "models/scientist_rosenberg.mdl");
		skipcosmeticstuff = true;
		m_iSciDialog = SCI_DIALOG_DEFAULT;
		m_eClass = SciClass_Rosenberg;
	}
	else if (FStrEq(STRING(pev->classname), "monster_scientist_russian"))
	{
		SET_MODEL(ENT(pev), "models/scientist2.mdl");
		skipcosmeticstuff = true;
		m_iSciCanHeal = false;
		m_iSciDialog = SCI_DIALOG_DEFAULT;
		m_eClass = SciClass_Russian;
	}
	else if (FStrEq(STRING(pev->classname), "monster_civilian_officeworker"))
	{
		SET_MODEL(ENT(pev), "models/npc-scientist.mdl");
		skipcosmeticstuff = true;
		m_iSciCanHeal = false;
		m_iSciDialog = SCI_DIALOG_DEFAULT;
		m_eClass = SciClass_CivOffice;
	}
	else if (FStrEq(STRING(pev->classname), "monster_civilian_businessman"))
	{
		SET_MODEL(ENT(pev), "models/businessman.mdl");
		skipcosmeticstuff = true;
		m_iSciCanHeal = false;
		m_iSciDialog = SCI_DIALOG_DEFAULT;
		m_eClass = SciClass_CivBusiness;
	}
	else if (FStrEq(STRING(pev->classname), "monster_civilian_woman"))
	{
		SET_MODEL(ENT(pev), "models/us_gal.mdl");
		skipcosmeticstuff = true;
		m_iSciCanHeal = false;
		m_iSciDialog = SCI_DIALOG_CIVILIAN_FEMALE;
		m_eClass = SciClass_CivWoman;
	}
	else if (FStrEq(STRING(pev->classname), "monster_civilian_schoolgirl"))
	{
		SET_MODEL(ENT(pev), "models/schoolgirl.mdl");
		skipcosmeticstuff = true;
		m_iSciCanHeal = false;
		m_iSciDialog = SCI_DIALOG_CIVILIAN_FEMALE;
		m_eClass = SciClass_CivSchoolgirl;
	}
	else
	{
		SET_MODEL(ENT(pev), "models/scientist.mdl");
		m_iSciDialog = SCI_DIALOG_DEFAULT;
		m_eClass = SciClass_Original;
	}
...
//	m_flDistTooFar		= 256.0;
	...
	if (!skipcosmeticstuff)
	{
		...
		{// -1 chooses a random head
			SetBodygroup(SCI_BODY_HEAD_GROUP, RANDOM_LONG(0, NUM_SCIENTIST_HEADS - 1));// pick a head, any head
		}

		// Luther is black, make his hands black
		if (m_iSciHead == SCI_BODY_HEAD_LUTHER)
			if (!m_iSkipBlackSkin)
				pev->skin = 1;
	}
...
{
	if (strcmp(STRING(pev->classname), "monster_rosenberg") == 0)
	{
	...
	else if (strcmp(STRING(pev->classname), "monster_scientist_russian") == 0)
	{
	...
	else if (strcmp(STRING(pev->classname), "monster_civilian_businessman") == 0)
	{
		PRECACHE_MODEL("models/businessman.mdl");
		PRECACHE_SOUND("scientist/sci_pain1.wav");
		PRECACHE_SOUND("scientist/sci_pain2.wav");
		PRECACHE_SOUND("scientist/sci_pain3.wav");
		PRECACHE_SOUND("scientist/sci_pain4.wav");
	...
	else if (strcmp(STRING(pev->classname), "monster_civilian_officeworker") == 0)
	{
	...
	else if (strcmp(STRING(pev->classname), "monster_civilian_woman") == 0)
	{
		PRECACHE_MODEL("models/us_gal.mdl");
		PRECACHE_SOUND("civilianwomen/sci_pain1.wav");
		PRECACHE_SOUND("civilianwomen/sci_pain2.wav");
		PRECACHE_SOUND("civilianwomen/sci_pain3.wav");
		PRECACHE_SOUND("civilianwomen/sci_pain4.wav");
		PRECACHE_SOUND("civilianwomen/sci_pain5.wav");
	}
	else if (strcmp(STRING(pev->classname), "monster_civilian_schoolgirl") == 0)
	{
...
void CScientist::TalkInit()
{
		...
		m_eClass == SciClass_Russian)
	{
		...
		m_szFriends[3] = "monster_scientist_russian";
		m_szFriends[4] = "monster_rosenberg";
		m_szFriends[5] = "monster_civilian_businessman";
		m_szFriends[6] = "monster_civilian_officeworker";
		m_szFriends[7] = "monster_civilian_woman";
		m_szFriends[8] = "monster_civilian_schoolgirl";
	} // Male civils will talk to friends in this order:
	else if (m_eClass == SciClass_CivBusiness ||
		m_eClass == SciClass_CivOffice)
	{
		m_szFriends[0] = "monster_civilian_businessman";
		m_szFriends[1] = "monster_civilian_officeworker";
		m_szFriends[2] = "monster_rosenberg";
		m_szFriends[3] = "monster_barney";
		m_szFriends[4] = "monster_scientist";
		m_szFriends[5] = "monster_scientist_russian";
		m_szFriends[6] = "monster_sitting_scientist";
		m_szFriends[7] = "monster_civilian_woman";
		m_szFriends[8] = "monster_civilian_schoolgirl";
	} // And female ones will talk to friends in this order:
	else if (m_eClass == SciClass_CivWoman ||
		m_eClass == SciClass_CivSchoolgirl)
	{
		m_szFriends[0] = "monster_civilian_woman";
		m_szFriends[1] = "monster_civilian_schoolgirl";
		m_szFriends[2] = "monster_civilian_businessman";
		m_szFriends[3] = "monster_civilian_officeworker";
		m_szFriends[4] = "monster_rosenberg";
		m_szFriends[5] = "monster_barney";
		m_szFriends[6] = "monster_scientist";
		m_szFriends[7] = "monster_scientist_russian";
		m_szFriends[8] = "monster_sitting_scientist";
	}

	/*
	monster_scientist
	monster_scientist_russian
	monster_rosenberg
	monster_civilian_businessman
	monster_civilian_officeworker
	monster_civilian_schoolgirl 
	monster_civilian_woman */

	// scientists speach group names (group names are in sentences.txt)
	if (m_iSciDialog == SCI_DIALOG_CIVILIAN_FEMALE)
	{
	m_szGrp[TLK_ANSWER]  =	"WO_ANSWER";
	m_szGrp[TLK_QUESTION] =	"WO_QUESTION";
	m_szGrp[TLK_IDLE] =		"WO_IDLE";
	m_szGrp[TLK_STARE] =	"WO_STARE";
	m_szGrp[TLK_USE] =		"WO_OK";
	m_szGrp[TLK_UNUSE] =	"WO_WAIT";
	m_szGrp[TLK_STOP] =		"WO_STOP";
	m_szGrp[TLK_NOSHOOT] =	"WO_SCARED";
	m_szGrp[TLK_HELLO] =	"WO_HELLO";

	m_szGrp[TLK_PLHURT1] =	"!WO_CUREA";
	m_szGrp[TLK_PLHURT2] =	"!WO_CUREB"; 
	m_szGrp[TLK_PLHURT3] =	"!WO_CUREC";

	m_szGrp[TLK_PHELLO] =	"WO_PHELLO";
	m_szGrp[TLK_PIDLE] =	"WO_PIDLE";
	m_szGrp[TLK_PQUESTION] ="WO_PQUEST";
	m_szGrp[TLK_SMELL] =	"WO_SMELL";
	
	m_szGrp[TLK_WOUND] =	"WO_WOUND";
	m_szGrp[TLK_MORTAL] =	"WO_MORTAL";
	}
	else 
	{
	...
	{
	default:
	case SCI_BODY_HEAD_GLASSES:	m_voicePitch = 105; break;	//glasses
	case SCI_BODY_HEAD_EINSTEIN: m_voicePitch = 100; break;	//einstein
	case SCI_BODY_HEAD_LUTHER:	m_voicePitch = 95;  break;	//luther
	case SCI_BODY_HEAD_SLICK:	m_voicePitch = 100;  break;	//slick
	}
}
...
void CScientist :: PainSound ( void )
{
	...
	{
	case 0: EMIT_SOUND_DYN( ENT(pev), CHAN_VOICE, "civilianwomen/sci_pain1.wav", 1, ATTN_NORM, 0, GetVoicePitch()); break;
	case 1: EMIT_SOUND_DYN( ENT(pev), CHAN_VOICE, "civilianwomen/sci_pain2.wav", 1, ATTN_NORM, 0, GetVoicePitch()); break;
	case 2: EMIT_SOUND_DYN( ENT(pev), CHAN_VOICE, "civilianwomen/sci_pain3.wav", 1, ATTN_NORM, 0, GetVoicePitch()); break;
	case 3: EMIT_SOUND_DYN( ENT(pev), CHAN_VOICE, "civilianwomen/sci_pain4.wav", 1, ATTN_NORM, 0, GetVoicePitch()); break;
	case 4: EMIT_SOUND_DYN( ENT(pev), CHAN_VOICE, "civilianwomen/sci_pain5.wav", 1, ATTN_NORM, 0, GetVoicePitch()); break;
	}
	}
	else 
	{
...
void CDeadScientist::KeyValue( KeyValueData *pkvd )
{
	...
	else if (FStrEq("clothes", pkvd->szKeyName))
	{
		m_iSciClothes = atoi(pkvd->szValue);
		pkvd->fHandled = true;
	}
	else if (FStrEq("prop", pkvd->szKeyName))
	{
		m_iSciProp = atoi(pkvd->szValue);
		pkvd->fHandled = true;
	}
	else
