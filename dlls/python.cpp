...
/*	Vector vecSrc	 = m_pPlayer->GetGunPosition( );
	...
	vecSpread.x = RANDOM_FLOAT(-1.0f, 1.0f);
	vecSpread.y = RANDOM_FLOAT(-1.0f, 1.0f);
	vecSpread.z = RANDOM_FLOAT(-1.0f, 1.0f);
	...
	CProjectileBullet *pBullet = CProjectileBullet::LaunchProjectile(WDT_357, m_pPlayer);
...
/*	EjectBrass(vecSrc + gpGlobals->v_forward * 8 - gpGlobals->v_up * 2,
...
//	PLAYBACK_EVENT_FULL( flags, m_pPlayer->edict(), m_usFirePython, 0.0, (float *)&g_vecZero, (float *)&g_vecZero, vecDir.x, vecDir.y, 0, 0, 0, 0 );
	...
	{
	case 1:	EMIT_SOUND_DYN(ENT(pev), CHAN_WEAPON, "weapons/357_shot1.wav", 1.0, ATTN_NORM, 0, 90 + RANDOM_LONG(-10, 20)); break;
	case 2:	EMIT_SOUND_DYN(ENT(pev), CHAN_WEAPON, "weapons/357_shot2.wav", 1.0, ATTN_NORM, 0, 90 + RANDOM_LONG(-10, 20)); break;
	}
