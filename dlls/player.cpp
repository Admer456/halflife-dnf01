...
void LinkUserMessages( void )
{
	...
	gmsgFlashlight = REG_USER_MSG("aFlashlight", 2);
	...
	gmsgKick = REG_USER_MSG( "LegDoKick", 0 );
	gmsgKickShake = REG_USER_MSG( "LegShake", 2 );
	gmsgPlayerTalk = REG_USER_MSG( "PlayerTalk", -1 );
...
// Player monologue sounds
char *szPlayerKickLines[] = 
{ 
	"player/talk/k_collatdmg.wav", 
	"player/talk/k_rleg.wav",
	"player/talk/k_nevergetsold.wav",
	"player/talk/k_wantsomebootty.wav"
};

char *szPlayerEmptyKickLines[] = 
{
	"player/talk/ke_exercise.wav",
	"player/talk/ke_gymnastics.wav",
	"player/talk/ke_why.wav"
};

char *szPlayerEmptyKickAirLines[] = 
{
	"player/talk/kea_ninja.wav",
	"player/talk/kea_flyingdickman.wav"
};

char *szPlayerRandomLines[] = 
{
	"player/talk/r_somethingsmells.wav",
	"player/talk/r_onceupon.wav",
	"player/talk/r_damn.wav",
	"player/talk/r_amistuck.wav",
	"player/talk/r_boring.wav"
};

char *szPlayerHurtLines[] =
{
	"player/talk/h_thinkthathurt.wav",
	"player/talk/h_bitch.wav",
	"player/talk/h_fleshwound.wav"
};

char *szPlayerAttackKillLines[] =
{
	"player/talk/ak_pussy.wav",
	"player/talk/ak_bullseye.wav",
	"player/talk/ak_dick1uglyass0.wav"
};
...
void CBasePlayer::PlayerMonologue( char **szPath, float flChance, int iMaxIndex, int iSpecificLine )
{
	if ( RANDOM_FLOAT( 0.0, 1.0 ) < flChance )
	{
		int iIndex = (iSpecificLine == -1) ? RANDOM_LONG( 0, iMaxIndex ) : iSpecificLine;

		EMIT_SOUND( ENT( pev ), CHAN_VOICE, szPath[ iIndex ], 1.0, ATTN_NONE );

		//MESSAGE_BEGIN( MSG_ONE, gmsgPlayerTalk, NULL, pev );
		//	WRITE_STRING( szPath[ iIndex ] );
		//MESSAGE_END();
	}
}
...
// fallpain sound, and damage will be inflicted based on how far the player fell
	...
	if ( pev->velocity.Length() < 10.0 )
		PlayerMonologue( szPlayerRandomLines, 0.001, 4, -1 );
...
//					if ( gun->m_flPumpTime != 1000 )
	...
	if (m_fKickTime >= 1 && m_fNoWeaponZone == false )
	{
/*		ALERT(at_console, "\nkick %i clock %i\tframetime %f",
			m_fKickTime,
			(int)(m_flKickClock * 10),
			gpGlobals->frametime); */

		if (m_fKickTime == 1 )
		{
			// clientside kicking animation
			MESSAGE_BEGIN(MSG_ONE, gmsgKick, NULL, edict());
			MESSAGE_END();

			// playing sound
			EMIT_SOUND_DYN(ENT(pev), CHAN_WEAPON, "weapons/cbar_miss1.wav", 1.0, ATTN_NORM, 0, 100);

			m_flKickClock = 0.7;	// stage 1. set the timer, then check if the timer is 0.3
			m_fKickTime = 2;		// switch to stage 2 - in the kick
		}							// cannot attack again in stage 2

		// switch to stage 3 in this time period - we have attacked, don't repeat the same attack every frame
		if (m_flKickClock < 0.54 && m_fKickTime != 3)
		{
			m_fKickTime = 3;

			MESSAGE_BEGIN(MSG_ONE, gmsgKickShake, NULL, pev);
				WRITE_COORD(3.0f);
			MESSAGE_END();

			TraceResult tr;

			Vector vecAngles = pev->v_angle;
			vecAngles.x = 8;

			UTIL_MakeVectors(vecAngles);
			Vector vecSrc = GetGunPosition() - gpGlobals->v_up * 16 + gpGlobals->v_forward * 16;
			...
			if ( CVAR_GET_FLOAT( "developer" ) )
			{
				...
				WRITE_BYTE( TE_LINE );
				WRITE_COORD_VECTOR( vecSrc );
				WRITE_COORD_VECTOR( vecEnd );
				WRITE_SHORT( 20 );
				WRITE_BYTE( 255 );
				WRITE_BYTE( 0 );
				WRITE_BYTE( 0 );
			...
			char **szWhichLineToPlay = szPlayerEmptyKickLines;
			if ( !(pev->flags & FL_ONGROUND) )
			{
				szWhichLineToPlay = szPlayerEmptyKickAirLines;
			}
			...
			pEntity->TraceAttack(pev, gSkillData.plrDmgCrowbar, gpGlobals->v_forward, &tr, DMG_CLUB);
			...
			if (pEntity)
			{
				if (pEntity->Classify() != CLASS_NONE && pEntity->Classify() != CLASS_MACHINE)
				{
					...
					fHitWorld = FALSE;
					szWhichLineToPlay = szPlayerKickLines;
				}
			}
			...
			PlayerMonologue( szWhichLineToPlay, 0.01, 3 );

			m_iWeaponVolume = flVol * 512;
		}
	}

	m_flKickClock -= gpGlobals->frametime;
	if (m_flKickClock < 0.02)
	{
		m_flKickClock = 0;
		m_fKickTime = 0; // once the kick has passed, reset to stage 0 and we can attack again :3
	}

	// *Should* revert us to a weapon we last had.
	if ( m_pActiveItem )
	{
		if ( !m_fNoWeaponZone && m_pActiveItem->m_iId == WEAPON_EMPTYHANDS )
		{
			SelectLastItem();
		}
		
		if (m_fNoWeaponZone != 2 )
			m_fNoWeaponZone = false;
	}
...
void CBasePlayer::SelectItem(const char *pstr)
{
	...
	if ( m_fNoWeaponZone >= 1 && !(FStrEq( pstr, "weapon_nothing" )) )
		return;
...
void CBasePlayer::CheatImpulseCommands( int iImpulse )
{
	...
	//	GiveNamedItem("weapon_nothing");
		...
		GiveAmmo(999, "556", M249_MAX_CARRY);
		GiveAmmo(999, "762", SNIPERRIFLE_MAX_CARRY);
		GiveAmmo(999, "spores", SPORELAUNCHER_MAX_CARRY);
		GiveAmmo(999, "shock", SHOCKRIFLE_MAX_CLIP);
...
#define clamp( val, min, max ) ( ((val) > (max)) ? (max) : ( ((val) < (min)) ? (min) : (val) ) )
	...
	// ????		Icons
		...
		for (i = 0; i < MAX_WEAPONS; i++)
		{
			...
			ALERT(at_console, "Listing weapon pszName: %s \nSlot %i \tPos %i \tId %i\n",
				   pszName,
				   II.iSlot,
				   II.iPosition,
				   II.iId);
...
BOOL CBasePlayer::HasActiveWeapon(void) 
{ 
	if (m_pActiveItem) 
		return TRUE; 
	else 
...
BOOL CBasePlayer::HolsterActiveWeapon(void) 
{ 
	if (HasActiveWeapon())
	{
		m_pActiveItem->Holster();
