...
void CHudAmmo::Think(void)
{
	...
	if ( gHUD.m_iWeaponBits != gWR.iOldWeaponBits )
	{
		...
		for (int i = MAX_WEAPONS-1; i > 0; i-- )
		{
			...
			if ( p )
			{
				gEngfuncs.Con_Printf( "gWR.GetWeapon() gives us %s at %i\n", p->szName, i );
...
int CHudAmmo::MsgFunc_WeaponList(const char *pszName, int iSize, void *pbuf )
{
	...
	if ( !strcmp( Weapon.szName, "weapon_nothing" ) )
		gEngfuncs.Con_Printf("MsgFunc_WeaponList\nweapon_nothing received, adding to to the weapon list\n");
