...
DECLARE_MESSAGE(m_aFlash, FlashBat)
DECLARE_MESSAGE(m_aFlash, aFlashlight)
...
int CHudFlashlight:: MsgFunc_aFlashlight(const char *pszName,  int iSize, void *pbuf )
{
...
void CHudFlashlight::drawNightVision()
{
	...
	if( m_nvSprite )
	{
		...
		for( auto x = 0; x < gHUD.m_scrinfo.iWidth; x += width )
		{
			...
			{
				drawingRect.top = 0;
				drawingRect.bottom = y - 10000; //+ height >= gHUD.m_scrinfo.iHeight ? gHUD.m_scrinfo.iHeight - y : height;
