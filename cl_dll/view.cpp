...
void V_CalcNormalRefdef ( struct ref_params_s *pparams )
{
	...
	// leg kicking view punch
	static float flCycle1 = 0;
	flCycle1 += pparams->frametime;
	if (flCycle1 > M_PI)
		flCycle1 = -M_PI;

	g_LegViewPunch /= 1.07;

//	gEngfuncs.Con_Printf("\ng_LVP: %f\t%f", g_LegViewPunch, flCycle1);
	// ----------------------
...
//	view->angles[PITCH] -= bob * 0.3;
	...
	view->curstate.angles.y += sin(-2 * flCycle1) * cos(8 * flCycle1) * g_LegViewPunch * 5;
	view->curstate.angles.z -= sin(flCycle1) * sin(-2 * flCycle1) * cos(8 * flCycle1) * g_LegViewPunch * 5;
	view->curstate.angles.z /= 1.01;

	Vector vecLegPunch = Vector(g_LegViewPunch * -sin(flCycle1) * 0.15,
		g_LegViewPunch * cos(4 * flCycle1) * 0.15,
		g_LegViewPunch * sin(2 * flCycle1) * 0.15);

	VectorAdd(pparams->cl_viewangles, vecLegPunch, pparams->cl_viewangles);

	pparams->cl_viewangles[2] /= 1.1;
...
# ifdef _TFC	// TFC models override HL models
		...
		{ "models/v_null.mdl",			"models/v_null.mdl"			},
		{ NULL, NULL } };
